...
test('renders title and author by default, but not url or likes', () => {
  ...
  render(
    <Blog blog={blog} updateBlogList={mockUpdate} currentUser={mockUser} />
  );

  expect(
    screen.getByText('Default Render Test Title Default Render Test Author')
  ).toBeDefined();
...
test('like button handler is called twice when like button is pressed twice', async () => {
  ...
  render(
    <Blog
      blog={blog}
      updateBlogList={mockUpdateHandler}
